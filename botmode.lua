...
-- Function to generate random direction with angles
local function getRandomDirection()
    -- Random angles for spherical coordinates
    local theta = math.random() * 2 * math.pi -- Horizontal angle (0 to 360 degrees)
    local phi = math.acos(2 * math.random() - 1) -- Vertical angle (0 to 180 degrees)
...
local function enableNoclip()
    ...
    connection = RunService.Heartbeat:Connect(function()
        ...
        if tick() - lastMove >= moveInterval then
            -- Get random direction with angles
            local randomDirection = getRandomDirection()
            local currentPosition = humanoidRootPart.Position
            local newPosition = currentPosition + (randomDirection * moveDistance)
